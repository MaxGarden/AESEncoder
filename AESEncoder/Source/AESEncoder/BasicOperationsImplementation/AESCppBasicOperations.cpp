...
void CAESCppBasicOperations::AddRoundKey(uint8_t* state, const uint8_t* expandedKey) const noexcept
{
...
void CAESCppBasicOperations::SubBytes(uint8_t* state) const noexcept
{
    for (auto i = 0u; i < s_StateDimension; ++i)
    {
...
void CAESCppBasicOperations::ShiftRows(uint8_t* state) const noexcept
{
...
void CAESCppBasicOperations::MixColumns(uint8_t* state) const noexcept
{
...
uint8_t CAESCppBasicOperations::GetSBoxValue(uint8_t index) const noexcept
{
    EDITOR_ASSERT(index < s_Sbox.size());
    return index < s_Sbox.size() ? s_Sbox[index] : 0u;
}

uint8_t CAESCppBasicOperations::GMultiplication(uint8_t first, uint8_t second) const noexcept
{
    uint8_t result = 0;
